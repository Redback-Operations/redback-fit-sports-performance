...
# Load the test data globally for use in the API
...
# Create a global FtpPredictor object
predictor = FtpPredictor(testdata)

@app.route('/fit', methods=['POST'])
def fit_model():
    predictor.fit(testdata)
    return jsonify({"message": "Model fitted successfully!"}), 200

@app.route('/predict', methods=['POST'])
def predict():
    try:
        # Get the data from the request body (assuming JSON input)
        ...
        # Drop 'FTP' and 'Activity Date' columns and handle missing values
        data = data.drop(['FTP', 'Activity Date'], axis=1, errors='ignore')
        data = data.fillna(0)
